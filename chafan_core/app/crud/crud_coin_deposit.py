import datetime
from typing import Any, Optional

from fastapi.encoders import jsonable_encoder
from sqlalchemy.orm import Session

from chafan_core.app import coins, models
from chafan_core.app.models.coin_deposit import CoinDeposit
from chafan_core.app.schemas.coin_deposit import CoinDepositCreate


def get(db: Session, id: Any) -> Optional[CoinDeposit]:
    return db.query(CoinDeposit).filter(CoinDeposit.id == id).first()


def make_deposit(
    db: Session,
    *,
    obj_in: CoinDepositCreate,
    authorizer_id: int,
    payee: models.User,
) -> CoinDeposit:
    coins.award_coins(db, payee, obj_in.amount, "coin_deposit")
    deposit = CoinDeposit(
        **jsonable_encoder(obj_in),
        created_at=datetime.datetime.now(tz=datetime.timezone.utc),
        authorizer_id=authorizer_id,
    )
    db.add(deposit)
    db.flush()
    db.refresh(deposit)
    return deposit


def get_with_ref_id(db: Session, *, ref_id: str) -> Optional[CoinDeposit]:
    return db.query(CoinDeposit).filter_by(ref_id=ref_id).first()
