...
def get(db: Session, id: Any) -> Optional[Comment]:
    return db.query(Comment).filter(Comment.id == id).first()


def get_by_uuid(db: Session, *, uuid: str) -> Optional[Comment]:
    return db.query(Comment).filter_by(uuid=uuid).first()
    ...
    if "parent_comment_uuid" in obj_in_data:
        if obj_in_data["parent_comment_uuid"]:
            parent_comment = unwrap(
                get_by_uuid(db, uuid=obj_in_data["parent_comment_uuid"])
            )
...
def cancel_upvote(db: Session, *, db_obj: Comment, voter: models.User) -> Comment:
    comment_upvote = (
        db.query(models.comment.CommentUpvotes)
        .filter_by(comment_id=db_obj.id, voter_id=voter.id)
        .first()
...
def get_all_valid(db: Session) -> List[Comment]:
